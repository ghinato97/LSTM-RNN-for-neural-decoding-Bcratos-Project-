...
if __name__=="__main__":
    ...
    model = Sequential()
    model.add(Bidirectional(LSTM(20, return_sequences = True), input_shape = (train_bin_set.shape[1], train_bin_set.shape[2])))
    ...
    keras.mixed_precision.set_global_policy('mixed_float16')
    model = Sequential()
    model.add(Bidirectional(LSTM(10, return_sequences = True), input_shape = (train_multi_set.shape[1], train_multi_set.shape[2])))
