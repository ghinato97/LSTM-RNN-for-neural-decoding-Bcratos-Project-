...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model", help="Name of BRNN model", default='MRec40_40_binned_spiketrains_PCA_Dataset_',
                        type=str)
    ...
    parser.add_argument("-d", "--dataset",help="Name of dataset from previous step, without extension", 
                        default='data/MRec41_40_binned_spiketrains/PCA_Dataset',
                        type=str)
    
    args = parser.parse_args()
    
    data_prefix = os.path.basename(os.path.normpath(args.model))
    path_multi=data_prefix+'binary_model'
    
    
    logging.info('Caricamento dataset')
    testset_path = os.path.join(args.dataset, 'binary_full.npz')
