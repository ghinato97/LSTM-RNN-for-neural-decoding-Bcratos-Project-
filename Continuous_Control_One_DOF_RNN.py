...
if __name__== "__main__":
    ...
    print('\n')
    model = Sequential()
    model.add(Bidirectional(LSTM(90, return_sequences = True, dropout=0.6), input_shape = (train_set.shape[1], train_set.shape[2])))
    model.add(Bidirectional(LSTM(units = 15, return_sequences = True, dropout=0.2)))
    model.add(Bidirectional(LSTM(units = 20, return_sequences = True, dropout=0.2)))
    model.add(Bidirectional(LSTM(units = 10, return_sequences = False, dropout=0.4)))
    model.add(Dense(1))
    model.summary()
    ...
    model.save(data_prefix+'_One_DOF_model')
