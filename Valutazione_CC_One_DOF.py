...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model", help="Name of CC BRNN model", default='Continuous_control_ZRec50_Mini_40_binned_spiketrains_One_DOF_model',
                        type=str)
    ...
    with open("y_test_ONE.pkl", "wb") as f:
    ...
    for i in range(len(lista_indici)):
        ...
        test=X_Test[n]
        label=y_test[n]
        one_dof=One_dof(label)
        
        predicted_value = model.predict(test)
        
        predicted_dict[str(n)]=predicted_value
        
        mse=mean_squared_error(one_dof[(Go):(End)],predicted_value[(Go):(End)])
    ...
    with open("predicted_value_ONE.pkl", "wb") as f:
    ...
    with open("sotto_soglia_value_ONE.pkl", "wb") as f:
