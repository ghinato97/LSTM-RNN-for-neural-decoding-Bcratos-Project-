...
def Prepare_Dataset(dirpath, df):
    lookback = 8
    x = []
    att0 = []  # attivazione obj_id 0
    att1 = []  # attivazione obj_id 1
    no_att = []  # no grasp
    ...
    for idx, row in df.iterrows():
        ...
        # colonna 0 : no grasp , colonna 1 : obj 0, colonna 2 : obj 1
    ...
    y0 = np.array(att0)
    y1 = np.array(att1)
    label = np.ones((y.shape[0], 3))
    label[:, 0] = y
    label[:, 1] = y0
    label[:, 2] = y1
    #data_set = somma_canali(x)
    # dimensionalmente [bin_in_considerazione,bin_precedenti,somma_sui_canali]
    data_set = np.array(x)
