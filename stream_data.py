...
#configurazione log per salvare e mostrare i risultati
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s:  %(message)s',
    datefmt='%m/%d/%Y %H:%M:%S',
    handlers=[
        logging.FileHandler("all_stream_data.log", mode='w'), #salva in un file chiamato all_stream_data-log
...
def from_matlab_to_numpy_data(filename, bin_size, out_prefix, not_only_correct=False):
    global seg
    #creo una cartella per i dati se non esiste 
    ...
    #creo un data frame vuoto ,righe e colonne
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset", help=".mat file containing brain recordings", default='INPUT/ZRec50_Mini.neo.mat',
                        type=str)
    ...
    x=from_matlab_to_numpy_data(args.dataset, args.bin_size, output_prefix)
